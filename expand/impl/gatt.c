...
static void print_service(struct service *service, const char *description)
{
	...
	if (!text)
		DEBUG("%s%s%s%s Service (Handle 0x%04x)\n\t%s\n\t"
					"%s\n",
					description ? "[" : "",
					description ? : "",
					description ? "] " : "",
...
static void print_inc_service(struct service *service, const char *description)
{
	...
	text = bt_uuidstr_to_str(service->uuid);
	if (!text)
		DEBUG("%s%s%s%s Included Service (Handle 0x%04x)\n\t"
					"%s\n\t%s\n",
					description ? "[" : "",
					description ? : "",
					description ? "] " : "",
...
static void print_chrc(struct chrc *chrc, const char *description)
{
	...
	if (!text)
		DEBUG("%s%s%sCharacteristic (Handle 0x%04x)\n\t%s\n\t"
					"%s\n",
					description ? "[" : "",
					description ? : "",
					description ? "] " : "",
...
static void print_desc(struct desc *desc, const char *description)
{
	...
	if (!text)
		DEBUG("%s%s%sDescriptor (Handle 0x%04x)\n\t%s\n\t"
					"%s\n",
					description ? "[" : "",
					description ? : "",
					description ? "] " : "",
...
static GDBusProxy *gatt_select_attribute(GDBusProxy *parent, const char *arg)
{
	if (arg[0] == '/')
...
static void read_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_message_iter_get_arg_type(&iter) != DBUS_TYPE_ARRAY) {
		DEBUG("Invalid response to read\n");
		return ;
	}
	...
	if (len < 0) {
		DEBUG("Unable to parse value\n");
		return ;
	}

	DEBUG_HEXDUMP(value, len);
				...
				struct write_attribute_data *data)
{
	...
	if (proxy == write_io.proxy &&
		...
		DEBUG("Attempting to write fd %d\n",
						io_get_fd(write_io.io));
		if (sock_send(write_io.io, &data->iov, 1) < 0) {
			DEBUG("Failed to write: %s", strerror(errno));
			return ;
		}
		return;
	}
...
static uint8_t *str2bytearray(char *arg, size_t *val_len)
{
	...
	for (i = 0; (entry = strsep(&arg, " \t")) != NULL; i++) {
		...
		if (!endptr || *endptr != '\0' || val > UINT8_MAX) {
			DEBUG("Invalid value at index %d\n", i);
			return NULL;
		}
...
static bool sock_read(struct io *io, void *user_data)
{
	...
	if (bytes_read < 0) {
		DEBUG("recvmsg: %s", strerror(errno));
		return false;
	}
	...
	if (chrc)
		...
		DEBUG("%s Notification:\n",
				g_dbus_proxy_get_path(notify_io.proxy));

	DEBUG_HEXDUMP(buf, bytes_read);
...
static bool sock_hup(struct io *io, void *user_data)
{
	...
	if (chrc) {
		DEBUG("Attribute %s %s sock closed\n", chrc->path,
...
static void acquire_write_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == TRUE) {
		DEBUG("Failed to acquire write: %s\n", error.name);
		dbus_error_free(&error);
		write_io.proxy = NULL;
		return ;
	}
	...
	if ((dbus_message_get_args(message, NULL, DBUS_TYPE_UNIX_FD, &fd,
		...
		DEBUG("Invalid AcquireWrite response\n");
		return ;
	}

	DEBUG("AcquireWrite success: fd %d MTU %u\n", fd,
								write_io.mtu);
...
void gatt_acquire_write(const char *uuid, const char *arg)
{
	...
	if (strcmp(iface, "org.bluez.GattCharacteristic1")) {
		DEBUG("Unable to acquire write: %s not a"
				" characteristic\n",
				g_dbus_proxy_get_path(proxy));
		return ;
	}
...
static void acquire_notify_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == TRUE) {
		DEBUG("Failed to acquire notify: %s\n", error.name);
		dbus_error_free(&error);
		write_io.proxy = NULL;
		return ;
	}
	...
	if ((dbus_message_get_args(message, NULL, DBUS_TYPE_UNIX_FD, &fd,
					...
					DBUS_TYPE_INVALID) == false)) {
		DEBUG("Invalid AcquireNotify response\n");
		return ;
	}

	DEBUG("AcquireNotify success: fd %d MTU %u\n", fd,
								notify_io.mtu);
...
void gatt_acquire_notify(const char *uuid, const char *arg)
{
	...
	iface = g_dbus_proxy_get_interface(proxy);
	if (strcmp(iface, "org.bluez.GattCharacteristic1")) {
		DEBUG("Unable to acquire notify: %s not a"
				" characteristic\n",
				g_dbus_proxy_get_path(proxy));
		return ;
	}
...
void gatt_notify_attribute(const char *uuid, bool enable)
{
	...
	DEBUG("Unable to notify attribute %s\n",
						g_dbus_proxy_get_path(proxy));
...
static void register_app_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == TRUE) {
		DEBUG("Failed to register application: %s\n",
				error.name);
		dbus_error_free(&error);
		return ;
	}

	DEBUG("Application registered\n");
...
void gatt_register_app(const char *uuid_array[], int uuid_count)
{
	...
	if (!l) {
		DEBUG("Unable to find GattManager proxy\n");
		return ;
	}

	for (i = 0; i < uuid_count; i++)
		uuids = g_list_append(uuids, g_strdup(uuid_array[i]));
	...
	if (uuids) {
		if (g_dbus_register_interface(conn, APP_PATH,
						...
						NULL) == FALSE) {
			DEBUG("Failed to register application"
					" object\n");
			return ;
		}
	}
	...
	if (g_dbus_proxy_method_call(l->data, "RegisterApplication",
						...
						NULL) == FALSE) {
		DEBUG("Failed register application\n");
		g_dbus_unregister_interface(conn, APP_PATH, PROFILE_INTERFACE);
		return ;
	}
}
...
static void unregister_app_reply(DBusMessage *message, void *user_data)
{
	...
	if (dbus_set_error_from_message(&error, message) == TRUE) {
		DEBUG("Failed to unregister application: %s\n",
				error.name);
		dbus_error_free(&error);
		return ;
	}

	DEBUG("Application unregistered\n");
...
void gatt_unregister_app(void)
{
	...
	if (g_dbus_proxy_method_call(l->data, "UnregisterApplication",
						...
						NULL) == FALSE) {
		DEBUG("Failed unregister profile\n");
		return ;
	}
}
...
void gatt_register_service(const char *uuid, uint16_t handle, bool isPrimary)
{
	...
	if (g_dbus_register_interface(conn, service->path,
					...
					service_free) == FALSE) {
		DEBUG("Failed to register service object\n");
...
void gatt_unregister_service(const char *uuid)
{
	...
	if (!service) {
		DEBUG("Failed to unregister service object\n");
		return ;
	}
...
void gatt_register_include(int argc, char *argv[])
{
	...
	if (!inc_service) {
		DEBUG("Failed to find  service object\n");
		return ;
	}
	...
	print_service(inc_service, NULL);
	print_inc_service(service, NULL);
...
void gatt_unregister_include(int argc, char *argv[])
{
	...
	if (!service) {
		DEBUG("Failed to unregister include service"
							" object\n");
		return ;
	}
	...
	if (!ser_inc) {
		DEBUG("Failed to find include service object\n");
		return ;
	}
							...
							void *user_data)
{
	...
	DEBUG("[%s (%s)] ReadValue: %s offset %u link %s\n",
							...
							void *user_data)
{
	...
	DEBUG("[%s (%s)] WriteValue: %s offset %u link %s\n",
			chrc->path, bt_uuidstr_to_str(chrc->uuid),
			path_to_address(device), offset, link);
...
static DBusMessage *create_sock(struct chrc *chrc, DBusMessage *msg)
{
	...
	DEBUG("Attribute %s %s sock acquired\n",
							...
							void *user_data)
{
	...
	chrc->notifying = true;
	DEBUG("Attribute %s (%s) notifications "
							...
							void *user_data)
{
	...
	DEBUG("Attribute %s (%s) indication confirm received",
			chrc->path, bt_uuidstr_to_str(chrc->uuid));
...
void gatt_register_chrc(const char *uuid, const char *flags, uint16_t data_max_len)
{
	...
	chrc->value = g_memdup(NULL, data_max_len);
	chrc->max_val_len = data_max_len;

	if (g_dbus_register_interface(conn, chrc->path, CHRC_INTERFACE,
					chrc_methods, NULL, chrc_properties,
					chrc, chrc_free) == FALSE) {
		DEBUG("Failed to register characteristic object\n");
		chrc_free(chrc);
		return ;
	}
...
void gatt_register_desc(const char *uuid, const char *flags, uint16_t data_max_len)
{
	...
	if (!service->chrcs) {
		DEBUG("No characteristic registered\n");
		return ;
	}
	...
	if (g_dbus_register_interface(conn, desc->path, DESC_INTERFACE,
		...
		DEBUG("Failed to register descriptor object\n");
		desc_free(desc);
		return ;
	}
